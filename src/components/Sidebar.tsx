...
export const Sidebar = ({ notes, selectedId, onSelect, onNew, isLoading }: SidebarProps) => {
  ...
  return (
    <div className="w-80 bg-slate-800/60 backdrop-blur-xl border-r border-slate-700/50 flex flex-col h-full shrink-0 select-none text-sm m-6 mr-0 rounded-2xl shadow-2xl">
      {/* Header */}
      <div className="p-6 border-b border-slate-700/50 flex justify-between items-center">
        <h1 className="font-bold text-gray-100 text-sm tracking-widest flex items-center gap-3">
          <span className="text-blue-400 text-lg">📚</span> KNOWLEDGE
        </h1>
        <button 
          onClick={onNew}
          ...
          Object.entries(tree).map(([subject, sections]) => (
            <div key={subject} className="mb-4">
              ...
              <div 
                ...
                className="flex items-center gap-3 p-3 hover:bg-slate-700/30 rounded-xl cursor-pointer text-slate-200 transition-all group"
              >
                <span className={`text-xs text-slate-400 transition-transform ${collapsed[subject] ? '-rotate-90' : ''}`}>▼</span>
                <FolderIcon />
                <span className="font-semibold text-sm uppercase tracking-wide group-hover:text-white">{subject}</span>
              </div>

              {!collapsed[subject] && (
                <div className="ml-4 border-l border-slate-600/30 pl-4 mt-2 space-y-2">
                  {Object.entries(sections).map(([section, sectionNotes]) => (
                    <div key={section}>
                      ...
                      <div 
                        ...
                        className="flex items-center gap-3 p-2 hover:bg-slate-700/20 rounded-lg cursor-pointer text-slate-300 hover:text-slate-100"
                      >
                        <span className={`text-xs text-slate-500 transition-transform ${collapsed[`${subject}-${section}`] ? '-rotate-90' : ''}`}>▼</span>
                        <SectionIcon />
                        <span className="text-sm font-medium">{section}</span>
                        <span className="text-xs bg-slate-700/50 px-2 py-1 rounded-full text-slate-400">{sectionNotes.length}</span>
                      </div>

                      {/* NOTES (Leaf) */}
                      {!collapsed[`${subject}-${section}`] && (
                        <div className="ml-6 space-y-1 border-l border-slate-600/20 pl-3">
                          {sectionNotes.map(note => (
                            <div
                              ...
                              className={`
                                ...
                                ${selectedId === note.id 
                                  ? 'bg-gradient-to-r from-blue-600/20 to-purple-600/20 text-blue-100 border border-blue-500/30' 
      ...
      {/* Footer Status */}
      <div className="p-4 border-t border-slate-700/50 bg-slate-900/30 text-xs text-slate-400 flex justify-between font-medium rounded-b-2xl">
