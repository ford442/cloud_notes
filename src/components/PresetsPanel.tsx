...
export const PresetsPanel = ({ onClose }: PresetsPanelProps) => {
  ...
  const [selectedDir, setSelectedDir] = useState<PresetDirName | null>(null);
  const [files, setFiles] = useState<PresetFileMeta[]>([]);
  const [filesLoading, setFilesLoading] = useState(false);
  const [filesError, setFilesError] = useState<string | null>(null);

  const [viewingFile, setViewingFile] = useState<string | null>(null); // filename
  ...
  const fetchDirs = useCallback(async () => {
    ...
    setDirsError(null);
    try {
      const data = await presetsAPI.listDirs();
      // Ensure all 5 dirs appear even if the backend omits ones with 0 files
  ...
  const handleSelectDir = useCallback(async (name: PresetDirName) => {
    setSelectedDir(name);
    setFiles([]);
    setFilesError(null);
    setViewingFile(null);
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          {!selectedDir ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-500 gap-4">
              ...
              <p className="text-lg font-medium">Select a preset directory</p>
              <p className="text-sm text-slate-600">Click a folder on the left to browse its files</p>
            </div>
          ) : (
            <>
              {/* File list */}
              <div className="border-b border-slate-700/60 overflow-auto" style={{ maxHeight: viewingFile ? '40%' : '100%' }}>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <button
                              ...
                              className="text-xs px-3 py-1.5 bg-slate-700 hover:bg-purple-700 text-slate-300 hover:text-white rounded-lg transition-colors font-medium"
                            >
                              View
                            </button>
                          </td>
              ...
              {viewingFile && (
                <div className="flex-1 flex flex-col overflow-hidden border-t border-slate-700/60">
                  <div className="px-6 py-2 bg-slate-800/80 flex items-center gap-3 shrink-0">
                    <span className="font-mono text-sm text-purple-300">{viewingFile}</span>
                    {fileLoading && (
                  ...
                  {fileContent !== null && (
                    <pre className="flex-1 overflow-auto p-6 text-xs font-mono text-slate-300 leading-relaxed whitespace-pre-wrap break-words bg-slate-950/60">
                      {fileContent}
                    </pre>
                  )}
                </div>
