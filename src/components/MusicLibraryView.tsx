...
export const MusicLibraryView = ({ onClose }: MusicLibraryViewProps) => {
  ...
  const syncLibrary = useCallback(async () => {
    ...
    setSyncing(true);
    try {
      const res = await fetch(`${flacApiUrl}/api/songs/scan`, { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        const added = data.added?.length ?? 0;
        addToast(added > 0 ? `Synced ${added} new track${added === 1 ? '' : 's'} from storage` : 'Library already up to date', 'success');
        await fetchSongs();
      } else {
        addToast('Sync failed — check storage backend', 'error');
      }
    } catch {
      addToast('Could not reach storage backend', 'error');
    } finally {
      setSyncing(false);
    }
  }, [flacApiUrl, addToast, fetchSongs]);
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={syncLibrary}
            disabled={syncing}
            className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-800 rounded-full shadow-md text-slate-500 hover:text-purple-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all text-sm font-medium"
            title="Scan storage for new tracks and add them to the library"
          >
            <svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24" className={syncing ? 'animate-spin' : ''}>
