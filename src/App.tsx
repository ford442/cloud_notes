...
function App() {
  ...
  // Editor mode state
  const [editorMode, setEditorMode] = useState<'simple' | 'rich' | 'graph' | 'canvas' | 'flashcards' | 'tasks' | 'named-notes' | 'library-browser' | 'music' | 'playlists' | 'mod-songs' | 'presets' | 'textures'>('rich')
    ...
    <div className={`h-screen w-screen overflow-hidden ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <SearchModal
        ...
        <div className="flex-1 flex flex-col h-full min-w-0 p-4 gap-4 relative transition-all duration-300">
          ...
          {/* Header Card */}
          <div className={`${isFocusMode || editorMode === 'named-notes' || editorMode === 'library-browser' || editorMode === 'music' || editorMode === 'playlists' || editorMode === 'mod-songs' || editorMode === 'presets' || editorMode === 'textures' ? 'hidden' : 'block'} bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 rounded-2xl p-4 shadow-2xl transition-colors duration-200 z-10`}>
          ...
          {/* Footer Card */}
          <div className={`${isFocusMode || editorMode === 'library-browser' || editorMode === 'named-notes' || editorMode === 'music' || editorMode === 'playlists' || editorMode === 'mod-songs' || editorMode === 'presets' || editorMode === 'textures' ? 'hidden' : 'block'} bg-white/60 dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200/50 dark:border-slate-700/50 rounded-2xl p-4 shadow-2xl transition-colors duration-200`}>
